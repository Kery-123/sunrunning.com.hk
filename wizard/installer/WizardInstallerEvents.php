...
class WizardInstallerEvents extends JPlugin
{
    ...
    public static function error($package, $msg)
    {
        ...
        <ul>
            <li class="wizardinstall-failure">
                ...
                <span class="wizardinstall-row"><?php echo ucfirst(trim($package['name'] . ' installation failed'));?></span>
    ...
    public static function installed($package)
    {
        ob_start();
        ?>
        <span class="wizardinstall-row"><?php echo ucfirst(trim($package['name'] . ' installed successfully'));?></span>
        <div style="margin: 20px 0 0 0;">Click the "Set Default" button to activate the template.</div>
        <?php
        $out = ob_get_clean();
    ...
    public static function updated($package)
    {
        ob_start();
        ?>
        <span class="wizardinstall-row"><?php echo ucfirst(trim($package['name'] . ' updated successfully'));?></span>
        <div style="margin: 20px 0 0 0;">Click the "Set Default" button to activate the template.</div>
        <?php
        $out = ob_get_clean();
